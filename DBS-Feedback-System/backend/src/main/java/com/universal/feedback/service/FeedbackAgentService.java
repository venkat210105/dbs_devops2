...
// import com.universal.util.NLPUtils;
...
public class FeedbackAgentService {
    ...
    // Universal Employee email for notifications
    @org.springframework.beans.factory.annotation.Value("${app.universal.employee.email:venkatmariserla02@gmail.com}")
    private String universalEmployeeEmail;
    ...
    private String assignTechGuy(Feedback feedback) {
        ...
        if (topic != null && topic.toLowerCase().contains("loan")) {
            return "loan.techguy@universal.com";
        } else if (service != null && service.toLowerCase().contains("credit")) {
            return "credit.techguy@universal.com";
        }
    ...
    public void processFeedback(Feedback feedback) {
        ...
        String universalEmailBody = String.format(
            "Feedback Alert!\n\n" +
            "Customer Name: %s\n" +
            "Customer Email: %s\n" +
            "Product/Service: %s\n" +
        ...
        String universalEmailBodyWithCalendar = universalEmailBody + (isCritical ? ("\n\n" + calendarText) : "\n\nNo meeting scheduled (feedback not critical).");
        try {
            emailService.sendEmail(
                universalEmployeeEmail, // Universal employee receives notification
                (isCritical ? "⚠ Critical Feedback Received" : "Feedback Received"),
                "From: " + senderEmail + "\nCustomer: " + String.valueOf(feedback.getEmail()) +
                "\nMessage: " + universalEmailBodyWithCalendar
            );
        } catch (Exception ex) {
        ...
        if (isCritical && feedback.getEmail() != null && !feedback.getEmail().isBlank()) {
            customerEmailBody = String.format(
                ...
                calendarText + "\n\n" +
                "Best regards,\n" +
                "Universal Customer Support Team",
                feedback.getCustomerName(),
                feedback.getServiceCategory(),
                reason,
                feedback.getComment(),
